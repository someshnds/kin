...
app()->booted(function () {
    ...
    if (is_plugin_active('ecommerce')) {
        ...
        shortcode()->setAdminConfig('product-category-products', function ($attributes) {
            ...
            return Theme::partial('shortcodes.product-category-products-admin-config',
            ...
            add_filter(SHORTCODE_REGISTER_CONTENT_IN_ADMIN, function ($data, $key, $attributes) {
                if (in_array($key, ['simple-slider'])) {
                    $ads = app(AdsInterface::class)->getModel()
                        ...
                        ->notExpired()
                        ->get();
                    $helper = __('Apply to Slide style 3');
                    return $data . Theme::partial('shortcodes.ads.config-in-admin', compact('ads', 'helper', 'attributes'));
                }
    ...
    add_shortcode('home-about', __('Home About'), __('Home About'), function () {
        return Theme::partial('shortcodes.home-about');
    });
